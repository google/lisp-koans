...
;; It is often helpful to dry run your macro (expand it without
;; executing it). The function macroexpand-1 comes in handy in this
;; situation.

(defmacro compound-call-1 (func-1 func-2 x)
  `(,func-1 (,func-2 ,x)))
...
;; Hint: Complex macros may involve control flow statements as well as
;; RECURSION.
...
(define-test test-compound-call
  ...
  (assert-equal '(zerop (compound-call 1+ 1- 1- 1+ 0))
		(macroexpand-1 '(compound-call zerop 1+ 1- 1- 1+ 0)))
  (assert-equal t (compound-call zerop 1+ 1- 1- 1+ 0)))

;; ----

;; It seems odd but backtick forms can reside in backtick forms. The key
;; to understanding hierarchical backtick forms is that 
;; (1) The outtermost commas belong to the innermost backtick.
;; (2) The outtermost backtick form gets expanded first.

(defmacro prefix-word-list (prefix &rest words)
  (let ((prefix-val (gensym)))
    `(let ((,prefix-val ',prefix))
       (list ,@(mapcar (lambda (word)
			 `(list ,prefix-val ',word))
		       words)))))
