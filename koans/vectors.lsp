...
(defun list-to-bit-vector (my-list)
  (let ((my-bit-vector (make-array (length my-list) :element-type 'bit)))
   (dotimes (i (length my-list) my-bit-vector)
    ;(assert-equal t (or (= (nth i my-list) 1) (= (nth i my-list) 0)))
    (setf (aref my-bit-vector i) (nth i my-list)))))
